...
namespace TrainingFPTCo.Models.Queries
{
    public class AccountQuery
    {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        accountDetail.RoleId = Convert.ToInt32(reader["RolesId"]);
                        // Lấy thông tin chi tiết của Role từ bảng Roles 
                        ...
                        accountDetail.Birthday = reader["Brithday"] != DBNull.Value ? Convert.ToDateTime(reader["Brithday"]) : (DateTime?)null;
                        ...
                        accountDetail.IPCleant = reader["IPClient"].ToString();
                        /*accountDetail.CreatedAt = Convert.ToDateTime(reader["CreatedAt"]);*/
                        accountDetail.UpdatedAt = reader["CreatedAt"] != DBNull.Value ? Convert.ToDateTime(reader["CreatedAt"]) : (DateTime?)null;
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        account.RoleId = Convert.ToInt32(reader["RolesId"]);
                        // Lấy thông tin chi tiết của Role từ bảng Roles (cần thay đổi tên cột nếu cần)
                        // Ví dụ: account.Role = GetRoleById(account.RoleId);
                        account.Role = GetRoleById(account.RoleId);
                        ...
                        if (!reader.IsDBNull(reader.GetOrdinal("Brithday")))
                        {
                            account.Birthday = Convert.ToDateTime(reader["Brithday"]);
                        }
                        ...
                        account.IPCleant = reader["IPClient"].ToString();
...
)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlUpdate = "UPDATE [Users] SET [RolesId] = @roleId, [Username] = @username, [Password] = @password, [ExtraCode] = @extraCode, [Email] = @email, [Phone] = @phone, [Address] = @address, [FullName] = @fullName, [FirstName] = @firstName, [LastName] = @lastName, [Brithday] = @birthday, [Gender] = @gender, [Education] = @education, [ProgramingLanguage] = @programingLanguage, [ToeicScore] = @toeicScore, [Skills] = @skills, [IPClient] = @ipCleant, [UpdatedAt] = @updatedAt WHERE [Id] = @id";
                connection.Open();
        ...
        internal IEnumerable<object> GetAllAccount(object value1, object value2)
        {
